function changeFormToRegister() {
    let formAuth = document.getElementById("form_auth");
...
function addScript(name: string) {
    ...
    xhr.onreadystatechange = () => {
        if (xhr.readyState === XMLHttpRequest.DONE) {
            let script = document.getElementsByName("script")[0];  // create a script DOM node
            script.innerText = (String(xhr.responseText));
        }
    }
}

function executeScript(name: string) {
    const xhr = new XMLHttpRequest();
    let loc = window.location.pathname;
    let dir = loc.substring(0, loc.lastIndexOf('/'));
    dir += '/app/page/assets/js/'+name + ".js?"+Math.random();
